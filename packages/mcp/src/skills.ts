...
function resolveSkillsDir(subdir: string): string {
  // Works for both run paths:
  //   - dist/mcp/index.js (tsup build): ../../skills/<subdir> -> <pkg>/skills/
  //   - src/mcp/index.ts (tsx dev):     ../../skills/<subdir> -> <pkg>/skills/
  const candidates = [
    path.resolve(__dirname, '..', '..', 'skills', subdir),
    // Legacy fallback: skills bundled under dist/ via copy-assets.
    path.resolve(__dirname, '..', 'skills', subdir),
  ];
